...
import { GET_CATEGORIES, GET_PRODUCTS, GET_OFFERS } from "../../graphql/queries";
...
export default function Offer() {
  ...
  const [setOffer, { error: errorMutationReview }] = useMutation(
    SET_OFFER,
    {refetchQueries: 
      [
        {query: GET_PRODUCTS},
  ...
  const { data:offerData, loading:offerLoading } = useQuery(GET_OFFERS,{ variables: { active: true}});
  ...
  return (
    <StyledOffer>
      ...
      <ul className="activeUsers">
        ...
        {
          !offerLoading  && offerData.getOffers.map( offer => 
            <li key={offer.id}>
            <span>
